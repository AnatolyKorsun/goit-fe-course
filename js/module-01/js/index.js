...
let userInput = prompt(
  "Сколько человек будет отдыхать с Вами",
  "от 6 до 25"
);
...
if (isValidInput) {
  ...
  if (touristPlacaes <= PLACES_TABA_TOUR) {
    ...
    } else window.alert(`Нам очень жаль приходите еще.`);
  } else if (touristPlacaes <= PLACES_SHARM_TOUR) {
    if (
      window.confirm(
  ...
  } else if (touristPlacaes <= PLACES_HURGADA_TOUR) {
    if (
      window.confirm(
  ...
  } else window.alert(`Извените мест ну вообще нет`);
